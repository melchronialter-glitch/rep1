...
export class PhoneBroker {
  ...
  async request(
    ...
    if (method === "chat.submit" && this.lineageFailure) {
      throw new RosyTalkBridgeError(
        "LINEAGE_UNAVAILABLE",
        "Submission is blocked because durable action lineage is unavailable; restart after repairing storage",
      );
    }
    ...
    const id = randomUUID();
    const request = relayRequestSchema.parse({ type: "request", id, method, params });
  ...
  private onMessage(socket: WebSocket, raw: string): void {
    ...
    try {
      ...
      this.assertTargetPackage(result.targetPackage);
