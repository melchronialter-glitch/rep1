...
- `Start-Aster-RosyTalk-Tunnel.cmd` is an optional helper for an already-created OpenAI Secure MCP Tunnel. It uses supported process-local environment configuration, including independent MCP discovery/call headers; it never creates a persistent tunnel profile, downloads `tunnel-client`, creates credentials, or installs a ChatGPT connection.
