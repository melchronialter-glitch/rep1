...
Aster RosyTalk Bridge carries bounded accessibility snapshots from one explicitly selected Android application package to an authorized MCP client. It can also submit text through that application's visible composer when the user enables a phone-side authorization that expires after 15 minutes.
...
[Visible window of selected Android package]
                  ...
                  ▼
[Android bridge + Keystore + 15-minute send authorization]
...
- The service examines only the current foreground window and refuses reads/submissions unless it finds exactly one enabled editable composer in the bottom 45% and at least 15% of the window width, visible context above it, and either one recognized Send control spatially adjacent to it (within one composer height/48 px vertically and a 25%-window horizontal gap) or a message/chat/composer/reply-signaled composer exposing `IME_ENTER`. It does not crawl Android storage, RosyTalk databases, other apps, notifications, contacts, the clipboard, or network traffic.
...
- **Allow message submission for 15 minutes** begins off, is never persisted, expires automatically, and is cleared by any relay disconnect, target-package change, or Accessibility-service stop.
...
- Submission performs two fresh captures plus an immediate private-signature recheck. It requires the selected package to be foreground, the caller's exact last-seen revision to remain current, an empty visible composer, and one adjacent unambiguous send mechanism. The bridge refuses stale, non-chat-like, drafted, or ambiguous UI instead of guessing.
...
- The relay retains the latest bounded snapshot/update state for read/wait behavior in memory and separately appends metadata-only observation/action ancestry to `.aster-runtime/rosytalk-lineage.jsonl`.
...
- v0.2 assumes one relay writer per lineage file and loads the active journal into memory. It never silently truncates provenance; stop the relay and externally checkpoint the journal/head before starting a new file when growth requires rotation.
- Five MCP tools are registered with distinct read/write annotations. Submission is not described as read-only.
...
- Keep the Android submission authorization off until the exact destination and text are ready; let it expire or disable/disconnect it immediately afterward.
