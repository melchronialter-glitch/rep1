...
Review these tools before enabling the connection:

| Tool | Effect |
| --- | --- |
| `rosytalk_status` | Reports phone, accessibility, target-package, and 15-minute submission-arming state. |
...
5. gives `tunnel-client` an environment-backed `Authorization` header for local `/mcp` discovery and calls;
...
`tunnel-client` changes over time. If the installed version rejects the environment-based configuration used by the helper, stop and follow `tunnel-client help quickstart` plus the current [official tunnel guide](https://developers.openai.com/api/docs/guides/secure-mcp-tunnels). Do not weaken relay authentication to make an incompatible client start.
...
4. Review the four discovered tools and their read/write annotations.
...
Keep the relay, Android connection, and `tunnel-client run` alive for discovery and every tool call. If the tunnel is missing, verify workspace association and Tunnels **Read + Use**. If discovery or calls fail, run `tunnel-client doctor --explain` again.
...
6. Submission is refused while the Android 15-minute arming switch is off or expired.
7. A successful submission is reported as submitted with delivery unconfirmed.
8. Closing the phone connection makes read, wait, and submit fail closed.
