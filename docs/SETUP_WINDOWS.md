...
Install the APK, then open **Aster RosyTalk Bridge**.
...
5. Enable **RosyTalk conversation bridge** under **Aster RosyTalk Bridge**.
6. Return to the bridge and tap **Connect**.
7. Leave **Allow message submission for 15 minutes** off while testing read-only access.
...
5. Make sure the composer is empty. Only then enable **Allow message submission for 15 minutes** and submit a harmless test message using the exact revision returned by the last read/wait.
...
- Turn the submission switch off; it also expires automatically after 15 minutes and is cleared on disconnect, target change, or Accessibility-service stop.
...
- Enable the phone-side 15-minute submission switch and confirm it has not expired.
